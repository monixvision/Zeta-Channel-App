        ...
        <div className="relative mx-auto bg-pink hover:bg-blue rounded-l-3xl rounded-br-3xl flex flex-col group transition-colors p-4 duration-300">
            ...
            <div className="flex gap-3 md:flex-col md:gap-5">
                <div className="flex flex-col gap-4 font-sans">
                    ...
                    <h1 className="text-[calc(1.5rem_+_0.5vw)] font-bold uppercase leading-[calc(1.5rem_+_0.5vw)]">{tituloEsp}</h1>


                    <ul className="flex flex-col gap-3 text-black hover:text-white font-pixel">
                        <li className="flex justify-between">
                            <p className="text-[calc(1.25rem_+_0.25vw)] font-sans text-black hover:text-white">{tipo}</p>
                            <p className="text-[calc(1.25rem_+_0.25vw)] text-black  hover:text-white font-pixel">{duracion} min</p>
                        </li>
                        <li className="w-full h-[2px] bg-black hover:bg-white"></li>
                        <li className="flex gap-2 md:justify-between lg:hidden">
                            <div>
                                <Btn text={`${dia} sep`} variant='solidblue' size='xs' font='pixel' />
                            </div>
                            <div className="flex gap-2 md:gap-4">
                                <div>
                                    <Btn text={horario[0]} variant={isHorario1Selected ? 'solidblack' : 'outlineblack'} size='xs' font='pixel' onClick={() => setIsHorario1Selected(!isHorario1Selected)}
                                    />
                                </div>
                                <div>
                                    <Btn text={horario[1]} variant={isHorario2Selected ? 'solidblack' : 'outlineblack'} size='xs' font='pixel' onClick={() => setIsHorario2Selected(!isHorario2Selected)} />
                                </div>
                            </div>
                        </li>
                        <li className="hidden flex gap-2 md:justify-between lg:block">
                            <div>
                                <Btn text={`${dia} sep`} variant='solidblue' size='xs' font='pixel' />
                            </div>
                            <div className="flex gap-2 md:gap-4">
